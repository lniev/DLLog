class BaseDiffLog {
			...
			if (this.validValue(curValue)) {
				preValue.forEach((itx, idx) => {
					const sameResult =
						this.getType(curValue) === "Array"
							? curValue.find((ity, idy) =>
									this.isEqual(key, itx, ity, idx, idy)
							  )
							: null;
				...
				this.getType(curValue) === "Array" &&
					curValue.forEach((itx, idx) => {
						const sameResult = preValue.find(ity =>
							this.isEqual(key, itx, ity)
						);
						if (!this.validValue(sameResult)) {
	...
	modifyPropertyLog(key, preValue, curValue, index) {
		return (
			this.getMapLabel(key, index) + "：由" + preValue + "修改为" + curValue
		);
	}
	...
	judgeAddOrDeletePropertyLog(key, value, index) {
		return this.reverse
			? this.addPropertyLog(key, value, index)
			: this.deletePropertyLog(key, value, index);
	}
	...
	isEqual(key, preValue, curValue, idx, idy) {
		if (this.logMap?.[key]?.$compareRule) {
			return this.logMap[key].$compareRule.call(
				this,
				key,
				preValue,
				curValue,
...
class DDLog extends BaseDiffLog {
	...
	getMapLabel(key, preValue, curValue, index) {
		...
		const handleValueMapTypes = {
			Object: () => {
				if (!this.validValue(keyMap.value))
					return [keyMap.label, preValue, curValue];
				return [
					keyMap.label,
		...
		if (handleValueMapTypes[keyMapTypes])
			return handleValueMapTypes[keyMapTypes].call(this);
